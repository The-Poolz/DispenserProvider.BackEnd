...
public class CreateAssetMessage : IValidatedMessage
{
    ...
    public AbstractMessage Eip712Message => Refund == null
        ? new CreateMessage(
            chainId: ChainId,
            poolId: PoolId,
            schedules: Schedules.Select(x => new ValidationSchedule(x.ProviderAddress, BigInteger.Parse(x.WeiRatio), x.StartDate.UtcDateTime, HandleFinishDate(x.FinishDate).UtcDateTime)),
    ...
    private static DateTime HandleFinishDate(DateTime? finishDate) => finishDate ?? DateTime.FromUnixTimeSeconds(0).UtcDateTime;
}